...
function App() {
  ...
  const exportToXML = (filename: string) => {
      const date = new Date()
      const tempTable = table.toSorted((a, b) => {return a.id - b.id})
      const resultData =
...
<CompetitionIndividuelle ID="Swiss" Date="${date.getDate().toString().padStart(2, "0")}.${(date.getMonth()+1).toString().padStart(2, "0")}.${date.getFullYear()}">
    <Tireurs>
        ${tempTable.map(fencer => {
            return `<Tireur ID="${fencer.id+1}" Nom="${fencer.lastName.toUpperCase()}" Prenom="${fencer.firstName}" Sexe="${fencer.gender}" Lateralite="D" Nation="NZL" Club="${fencer.club}" Classement="${fencer.rank}" Statut="N"></Tireur>`
        }).join("\n")}
    </Tireurs>
    <Arbitres />
    <Phases>
