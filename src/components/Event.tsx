        ...
        <div className={"grid lg:grid-cols-2"}>
            ...
            <div className={"max-lg:ml-8 " + (printTarget === "bouts" ? "print-visible" : "print:hidden")}>
                ...
                <form id={"roundForm"} onSubmit={endRound} className={"m-2 "}>
                    {props.competition.events[props.eventIndex].roundNum > 0 && props.competition.events[props.eventIndex].rounds[activeRound] && props.competition.events[props.eventIndex].rounds[activeRound].bouts.map((bout, index) => (
                        <div className={"p-4 print:w-full"} key={index}>
                            ...
                            <input type={"number"} className={"w-8 p-1 justify-items-center border-2 border-black rounded ml-8 " + (bout.winner !== -1 ? bout.winner === bout.fencer2.id ? "text-green-800 border-green-600 border-[3px]" : "text-red-700 border-red-600" : "")}
                                   min={0} id={bout.fencer2.id.toString() + "_score"} style={{marginLeft: '4px'}} required value={bout.score2 ?? ""} onInput={(event) => updateBoutScore(bout.id, undefined, event.currentTarget.valueAsNumber)} disabled={activeRound !== props.competition.events[props.eventIndex].rounds.length - 1}/>
                    ...
                    {props.competition.events[props.eventIndex].roundNum > 0 ? <button type={"button"} onClick={() => print("bouts")} className={"border rounded px-1 mt-2 print:hidden flex"}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6 w-4 h-4 mt-auto mb-auto">
                    ...
                    {started && activeRound === props.competition.events[props.eventIndex].rounds.length - 1 ? <button type={"submit"} className={"border rounded px-2 mt-6 print:hidden"}>End Round</button> : ""}
                    {started && activeRound === props.competition.events[props.eventIndex].rounds.length - 1 ? <Dialog.Root>
                        <Dialog.Trigger>
                            <a type={"button"}
